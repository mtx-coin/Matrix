...
MATRIX Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9169 for connections and services them.
...
However, in open source software such as MATRIX Core, where everyone uses
...
pay attention to for reviewers of MATRIX Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MATRIX Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MATRIX Core so that they can be integrated
