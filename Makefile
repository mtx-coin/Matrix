...
pkgdatadir = $(datadir)/matrixcore
pkgincludedir = $(includedir)/matrixcore
pkglibdir = $(libdir)/matrixcore
pkglibexecdir = $(libexecdir)/matrixcore
...
CONFIG_HEADER = $(top_builddir)/src/config/matrix-config.h
CONFIG_CLEAN_FILES = libmatrixconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libmatrixconsensus.pc.in \
	...
	$(top_srcdir)/src/config/matrix-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/MATRIX/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/MATRIX/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/MATRIX/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/MATRIX/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = matrix-cli
BITCOIN_DAEMON_NAME = matrixd
BITCOIN_GUI_NAME = matrix-qt
BITCOIN_TX_NAME = matrix-tx
...
COPYRIGHT_HOLDERS_FINAL = The MATRIX Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = MATRIX Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/MATRIX/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = matrixcore
PACKAGE_BUGREPORT = https://github.com/matrixpay/matrix/issues
PACKAGE_NAME = MATRIX Core
PACKAGE_STRING = MATRIX Core 1.0.0
PACKAGE_TARNAME = matrixcore
PACKAGE_URL = https://matrix.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/MATRIX
abs_srcdir = /tmp/MATRIX
abs_top_builddir = /tmp/MATRIX
abs_top_srcdir = /tmp/MATRIX
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/MATRIX/build-aux/install-sh
...
pkgconfig_DATA = libmatrixconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/matrix-cli.bash-completion \
	       $(top_srcdir)/contrib/matrix-tx.bash-completion \
	       $(top_srcdir)/contrib/matrixd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_matrix_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_matrix_coverage.info test_matrix.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/MATRIX-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/matrix-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/matrix-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/matrix-config.h
$(top_srcdir)/src/config/matrix-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/matrix-config.h src/config/stamp-h1
libmatrixconsensus.pc: $(top_builddir)/config.status $(srcdir)/libmatrixconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/MATRIX-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/MATRIX-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/MATRIX-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_matrix.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_matrix -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_matrix_filtered.info: test_matrix.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_matrix_filtered.info
...
#test_matrix_coverage.info: baseline_filtered_combined.info test_matrix_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_matrix_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_matrix_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_matrix_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_matrix.coverage/.dirstamp:  test_matrix_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_matrix.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_matrix.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
